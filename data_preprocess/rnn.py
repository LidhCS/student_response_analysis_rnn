...
def build_nn_data(data, num_questions, use_correct=True, use_hints=False):
    ...
    """
    all_users_data = []
    data.sort_values([USER_IDX_KEY, TIME_IDX_KEY], inplace=True)
    ...
    for user_id, user in data.groupby(USER_IDX_KEY):
        ...
        for this_skill, next_skill, this_correct, next_correct, hint in zip(
            ...
            # y.append(next_skill)
            y.append(next_skill + num_questions * next_correct * (hint == 0) * use_correct)
